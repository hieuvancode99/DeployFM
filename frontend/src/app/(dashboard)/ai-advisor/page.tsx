    ...
    <div className="space-y-4 text-slate-300 leading-relaxed text-sm">
      {lines.map((line, index) => {
        ...
        if (trimmed.startsWith('### ')) {
          return (
            <h3 key={index} className="text-lg font-bold text-white mt-5 mb-2 flex items-center gap-2">
        ...
        if (trimmed.startsWith('#### ')) {
          return (
            <h4 key={index} className="text-sm font-bold text-slate-100 uppercase tracking-wider mt-4 mb-2">
              {trimmed.substring(5)}
            </h4>
          );
        }
        ...
        if (trimmed.startsWith('* ') || trimmed.startsWith('- ')) {
          ...
          return (
            <li key={index} className="list-disc list-inside ml-4 text-slate-300 mt-1" dangerouslySetInnerHTML={{ __html: formatted }}></li>
          );
        }
        ...
        if (/^\d+\.\s/.test(trimmed)) {
          ...
          return (
            <div key={index} className="flex gap-2.5 ml-2 mt-1.5">
              <span className="font-bold text-indigo-400">{trimmed.match(/^\d+/)![0]}.</span>
              <span className="text-slate-300" dangerouslySetInnerHTML={{ __html: formatted }}></span>
            </div>
          );
        }
        ...
        return (
          <p key={index} className="text-slate-300" dangerouslySetInnerHTML={{ __html: formattedParagraph }}></p>
        );
      })}
    </div>
  );
...
function parseInlineFormatting(text: string) {
  ...
  // Inline code `code`
  res = res.replace(/`(.*?)`/g, '<code class="bg-slate-950 border border-slate-800 rounded px-1.5 py-0.5 text-xs text-indigo-400 font-mono">$1</code>');
  return res;
}
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <p className="text-slate-400 mt-1">Cung cấp báo cáo phân tích chi tiêu cá nhân thông minh và các giải pháp tiết kiệm hiệu quả từ Trí tuệ Nhân tạo.</p>
      </div>

      <div className="grid grid-cols-1 gap-6">
        {/* Intro Banner */}
        <div className="relative overflow-hidden rounded-2xl border border-slate-800 bg-indigo-950/20 p-6 shadow-xl backdrop-blur-md">
          ...
          <div className="flex gap-4 items-start z-10 relative">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Bạn muốn tối ưu hóa chi tiêu trong tháng?</h2>
        ...
        {(loading || advice) && (
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-6 shadow-xl backdrop-blur-md min-h-[300px]">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-64 gap-3 text-slate-400">
                <div className="relative">
                  <div className="h-12 w-12 animate-spin rounded-full border-4 border-indigo-600 border-t-transparent"></div>
                  <BrainCircuit className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-5 w-5 text-indigo-400 animate-pulse" />
                </div>
                <p className="text-sm font-semibold text-slate-200 mt-2">Cố vấn AI đang xử lý giao dịch của bạn...</p>
              ...
              <div className="space-y-6">
                <div className="flex items-center gap-2.5 pb-4 border-b border-slate-800">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-white">Trí tuệ nhân tạo Finance Advisor</h3>
                    <p className="text-[10px] text-slate-400">Phản hồi dựa trên dữ liệu thu chi thực tế của bạn</p>
                  </div>
                </div>
